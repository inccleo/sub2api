...
- 模型：`jev-latest`、`jev-preview`、`jev-1.13.0`，TypeSafe 分组的 `/v1/models` 也会返回这些模型
...
Jev 内置价格为输入 `$0.042/百万 tokens`、输出 `$0`，渠道定价可以覆盖。凭据、限流、过载、服务端和网络错误沿用现有账号冷却与故障切换链路；请求错误（`400`、`422`）不会切换账号重试。
