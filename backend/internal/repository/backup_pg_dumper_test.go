...
func TestPgDumperRestoreHoldsMigrationLockThroughProcessExit(t *testing.T) {
	var mock sqlmock.Sqlmock
	commandCreated := false
	tempDir := t.TempDir()
	readyPath := tempDir + "/ready"
	releasePath := tempDir + "/release"
	dumper, createdMock := newTestPgDumper(t, func(ctx context.Context, name string, args ...string) *exec.Cmd {
		commandCreated = true
		...
		expectBackupMigrationUnlock(mock)
		return exec.CommandContext(ctx, "sh", "-c", `touch "$1"; while [ ! -e "$2" ]; do sleep 0.01; done; cat >/dev/null`, "restore-test", readyPath, releasePath)
	})
	mock = createdMock
	expectBackupMigrationLock(mock)
	...
	go func() {
		done <- dumper.Restore(context.Background(), strings.NewReader("restore-data"))
	}()
	require.Eventually(t, func() bool {
		_, err := os.Stat(readyPath)
		return err == nil
	}, 2*time.Second, 10*time.Millisecond)
	require.Error(t, mock.ExpectationsWereMet(), "migration lock must remain held while psql is running")
...
func TestPgDumperRestoreReleasesMigrationLockWhenProcessFails(t *testing.T) {
	dumper, mock := newTestPgDumper(t, func(ctx context.Context, _ string, _ ...string) *exec.Cmd {
		return exec.CommandContext(ctx, "sh", "-c", "cat >/dev/null; printf restore-failed; exit 7")
	})
	expectBackupMigrationLock(mock)
	expectBackupMigrationUnlock(mock)
...
func TestPgDumperRestoreReportsProcessAndUnlockFailures(t *testing.T) {
	dumper, mock := newTestPgDumper(t, func(ctx context.Context, _ string, _ ...string) *exec.Cmd {
		return exec.CommandContext(ctx, "sh", "-c", "printf restore-failed; exit 7")
	})
	expectBackupMigrationLock(mock)
	mock.ExpectExec(regexp.QuoteMeta("SELECT pg_advisory_unlock($1)")).
		WithArgs(migrationsAdvisoryLockID).
