...
func TestForwardSystemOneForwardsNativeProtocolAndUsage(t *testing.T) {
	requestBody := []byte(`{"model":"jev-1.13.0","state":{"text":"sample"},"questions":{"q":{"type":"choice","instructions":"Pick","criteria":{"a":"A","b":"B"}}}}`)
...
func TestForwardSystemOneAllowsSuccessfulResponseWithoutModel(t *testing.T) {
	...
	result, err := svc.ForwardSystemOne(context.Background(), newSystemOneTestContext(), account, []byte(`{"model":"jev-latest"}`))
	require.NoError(t, err)
	require.Equal(t, responseBody, result.Body)
