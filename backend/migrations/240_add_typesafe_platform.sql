-- Add TypeSafe / Jev as a first-class platform.
--
...
--
-- Runs after 238_opencode_go_platform.sql (and 239_daily_checkins.sql, which
-- does not touch these CHECKs). DROP ... IF EXISTS + 幂等守卫保证可重入；
-- 新约束是 238 的超集，必须同时保留 OpenCode Go。
...
BEGIN
    ...
    IF monitor_constraint_def IS NULL OR position('typesafe' IN monitor_constraint_def) = 0 THEN
        ALTER TABLE channel_monitors
            DROP CONSTRAINT IF EXISTS channel_monitors_provider_check;
        ALTER TABLE channel_monitors
            ADD CONSTRAINT channel_monitors_provider_check
            CHECK (provider IN ('openai', 'anthropic', 'gemini', 'grok',
                                'antigravity', 'kimi', 'zhipu', 'deepseek', 'minimax', 'opencode_go', 'typesafe'));
    END IF;
    ...
    IF template_constraint_def IS NULL OR position('typesafe' IN template_constraint_def) = 0 THEN
