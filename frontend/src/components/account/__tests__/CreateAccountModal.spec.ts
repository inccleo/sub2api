...
const KimiDeviceFlowStub = defineComponent({
  name: 'KimiDeviceFlow',
  template: '<div data-testid="kimi-device-flow" />',
})
  ...
  return mount(CreateAccountModal, {
    ...
    global: {
      stubs: {
        ...
        KimiDeviceFlow: KimiDeviceFlowStub,
        ConfirmDialog: true,
        Select: true,
        Icon: true,
        PlatformIcon: true,
...
describe('CreateAccountModal OpenAI long-context billing', () => {
  ...
  it('keeps Kimi device-flow OAuth available alongside API key accounts', async () => {
    const wrapper = mountModal()
    await selectButtonByText(wrapper, 'admin.accounts.types.kimiOauth')
    await wrapper.get('form#create-account-form input[type="text"]').setValue('Kimi OAuth')
    await wrapper.get('form#create-account-form').trigger('submit.prevent')

    expect(wrapper.getComponent(KimiDeviceFlowStub).exists()).toBe(true)
  })
