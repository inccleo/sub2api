...
describe('PlazaModelPricingTable', () => {
  ...
  it('仅配置区间倍率时按基础价格展示 input/output/cache 各档价格', () => {
    ...
    expect(text).toContain('$2.8571')
    expect(text).toContain('$10.7143')
    expect(text).toContain('$3.5714')
    expect(text).toContain('$0.5714')
    ...
    expect(cells[3].text()).toContain('$1.7857')
    expect(cells[3].text()).toContain('$0.2857')
    expect(cells[3].text()).toContain('$3.5714')
    expect(cells[3].text()).toContain('$0.5714')
  })
...
describe('PlazaModelPricingTable 长上下文阶梯', () => {
  ...
  it('实付缓存列按档分行并乘倍率,每档一行与输入/输出列对齐;档位标签只在输入列', () => {
    ...
    expect(rows).toHaveLength(2)
    // 写 6.25 × 0.5 / 读 0.5 × 0.5;高档 12.5 × 0.5 / 1 × 0.5,再按展示汇率折算
    expect(rows[0].text()).toContain('modelPlaza.table.cacheWriteShort')
    expect(rows[0].text()).toContain('$0.4464')
    expect(rows[0].text()).toContain('$0.0357')
    expect(rows[1].text()).toContain('$0.8929')
    expect(rows[1].text()).toContain('$0.0714')
...
describe('PlazaModelPricingTable 分时计价', () => {
  ...
  it('有分时倍率的模型展开为标准行 + 每时段一行,时段行价格按倍率折算且倍率列显示生效倍率', () => {
    ...
    expect(baseCells[1].text()).toContain('$0.3429')
    expect(baseCells[7].text()).toContain('0.1143x')
    ...
    expect(nightCells[1].text()).toContain('$0.1714')
    expect(nightCells[2].text()).toContain('$0.8571')
    expect(nightCells[3].text()).toContain('$0.2143')
    expect(nightCells[7].text()).toContain('0.0571x')

    // 晚高峰行:3 × 0.8 × 1.2 = 2.88,倍率 0.96x
    const peakCells = trs[2].findAll('td')
    expect(peakCells[0].text()).toContain('18:00–22:00')
    expect(peakCells[1].text()).toContain('$0.4114')
    expect(peakCells[7].text()).toContain('0.1371x')
