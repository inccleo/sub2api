...
const accountSupportsBatchUsage = (account: Account) => {
  ...
  if (account.platform === 'kimi') return account.type === 'oauth'
  return false
}
...
const handleAccountUsageLoaded = (account: Account, usage: AccountUsageInfo) => {
  if (usageBatchByAccountId.value[String(account.id)] !== usage) {
    setUsageBatchState(account.id, usage, null)
  }
  if (account.platform !== 'kimi') return
